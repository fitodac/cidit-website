import React, { useEffect, useRef, useState, type CSSProperties } from "react";
import "plyr/dist/plyr.css";

const PLYR_COLOR_MAIN = "#E15D3B";
const YOUTUBE_ID = "QvlbWnUcctg";

export default function Video() {
	const containerRef = useRef<HTMLDivElement | null>(null);
	const playerElRef = useRef<HTMLDivElement | null>(null);
	const playerRef = useRef<any | null>(null);
	const [overlayVisible, setOverlayVisible] = useState(true);

	useEffect(() => {
		let cancelled = false;

		const setup = async () => {
			const PlyrModule = await import("plyr");
			const PlyrLib = (PlyrModule as any).default ?? PlyrModule;
			if (!playerElRef.current || cancelled) return;

			const player = new PlyrLib(playerElRef.current, {
				muted: true,
				loop: { active: true },
				controls: ["play", "progress"],
				clickToPlay: true,
			});

			playerRef.current = player;

			player.on("ready", () => {
				try {
					player.muted = true;
					player.play();
				} catch {}
			});

			player.on("ended", () => {
				try {
					player.restart();
					player.play();
				} catch {}
			});
		};

		void setup();

		return () => {
			cancelled = true;
			if (playerRef.current) {
				playerRef.current.destroy();
				playerRef.current = null;
			}
		};
	}, []);

	const style: CSSProperties = {
		"--plyr-color-main": PLYR_COLOR_MAIN,
	} as CSSProperties;

	const handleUnmute = () => {
		const player = playerRef.current;
		if (!player) return;

		try {
			player.restart();
			player.muted = false;
			player.play();
		} catch {}

		setOverlayVisible(false);
	};

	return (
		<div className="mxd-section padding-pre-title">
			<div className="mxd-container">
				<div className="mxd-divider">
					<div className="mxd-divider__video" ref={containerRef} style={style}>
						<div
							id="player"
							ref={playerElRef}
							className="w-100 vh-100"
							data-plyr-provider="youtube"
							data-plyr-embed-id={YOUTUBE_ID}
						/>
					</div>

					{overlayVisible && (
						<div className="mxd-divider__trigger">
							<button
								type="button"
								onClick={handleUnmute}
								className="btn-rotating btn-rotating-blur-outline showreel-trigger"
								aria-label="Activar audio del video"
							>
								<svg
									xmlns="http://www.w3.org/2000/svg"
									viewBox="0 0 24 24"
									fill="currentColor"
									className="size-10! md:size-32!"
								>
									<path d="M13 7.22056L9.60282 10.0001H6V14.0001H9.60282L13 16.7796V7.22056ZM8.88889 16.0001H5C4.44772 16.0001 4 15.5524 4 15.0001V9.00007C4 8.44778 4.44772 8.00007 5 8.00007H8.88889L14.1834 3.66821C14.3971 3.49335 14.7121 3.52485 14.887 3.73857C14.9601 3.8279 15 3.93977 15 4.05519V19.9449C15 20.2211 14.7761 20.4449 14.5 20.4449C14.3846 20.4449 14.2727 20.405 14.1834 20.3319L8.88889 16.0001ZM18.8631 16.5911L17.4411 15.1691C18.3892 14.4376 19 13.2902 19 12.0001C19 10.5697 18.2493 9.31476 17.1203 8.60766L18.5589 7.16906C20.0396 8.26166 21 10.0187 21 12.0001C21 13.8422 20.1698 15.4905 18.8631 16.5911Z" />
								</svg>
							</button>
						</div>
					)}
				</div>
			</div>
		</div>
	);
}
